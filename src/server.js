...
async function next(payload) {
  ...
  const r = await fetch(`${NEXT_STAGE_URL}/process`, { method: "POST", headers: { "content-type": "application/json", ...(PIPELINE_SECRET ? { "x-pipeline-secret": PIPELINE_SECRET } : {}) }, body: JSON.stringify(payload) });
...
async function generateImage(job) {
  ...
  if (!r.ok) { const detail = await r.text().catch(() => ""); throw new Error(`Pollinations returned ${r.status}${detail ? `: ${detail.slice(0, 500)}` : ""}`); }
...
app.get("/health", (_req, res) => res.json({ ok: true, stage: STAGE_NAME, nextStageConfigured: Boolean(NEXT_STAGE_URL) }));
...
app.post("/process", async (req, res) => {
  try {
    ...
    if (STAGE_NAME === "generation") { const image = await generateImage(job); return res.json({ ok: true, ...job, ...image }); }
    if (STAGE_NAME === "result-validator") { if (!job.imageData) return res.status(502).json({ ok: false, message: "Generation stage returned no image." }); return res.json({ ok: true, validated: true, ...(await next(job)) }); }
    if (STAGE_NAME === "asset-proxy") return res.json({ ok: true, imageData: job.imageData, jobId: job.jobId, prompt: job.prompt, ...(await next(job)) });
    ...
    res.status(500).json({ ok: false, stage: STAGE_NAME, message: error?.message || "Stage failed." });
  }
});
